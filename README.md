...
| 팀원 A | 게시판 | 공지사항·일반 게시판 CRUD, 댓글, 첨부파일, 게시글·댓글 관리(인사팀) |
| 팀원 B | 문서 / 전자결재 | 문서 작성·결재라인 설정, 승인·반려 처리, 결재담당자 관리, 문서 관리(인사팀) |
| 팀원 C | 업무 | 회사·부서·사원 업무 등록, 업무 지시·수신, 일정 캘린더, 업무 관리(인사팀) |
...
**구현 엔드포인트** — `/login.do` (GET: 폼 표시 / POST: 로그인 처리)
...
**구현 엔드포인트 9개** — 목록 · 검색 · 등록(폼/처리) · 수정(폼/처리) · 퇴사(확인/실행 2단계) · 직원 목록 JSON API
...
**구현 엔드포인트 5개** — 부서 목록 · 목록 JSON · 등록 · 수정 · 삭제
...
**구현 엔드포인트 3개** — 출근 기록 · 퇴근 기록 · 전체 조회(인사부 전용)
...
**구현 엔드포인트 5개** — 업무 종류 JSON · 회사/부서 업무 검색 · 회사 업무 등록 · 부서 업무 배정
