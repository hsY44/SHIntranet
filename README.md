...
3. [팀원 역할 분담](#팀원-역할-분담)
4. [테스트 계정](#테스트-계정)
5. [내 담당 파트 (유현선)](#내-담당-파트-유현선)
6. [Oracle 주요 프로시저](#oracle-주요-프로시저)
...
| 팀원 | 담당 영역 | 주요 기능 |
|------|-----------|-----------|
| 유현선 | 인사 / 출퇴근 / 업무 등록 | 로그인, 직원·부서·직급 관리, 출퇴근 기록, 업무 등록·조회 |
...
```
[1단계] 사원코드 입력 → VIEW_EMPLOYEE(재직자 뷰) 조회
        · 뷰에 없고 EMP 원본에 있으면  →  "이미 퇴사 처리된 사원"
        · EMP 원본에도 없으면          →  "존재하지 않는 사원코드"

[2단계] 정보 확인 후 EMP_EXIT 프로시저 실행
...
|------|-----------|
| 등록 | 동일 부서명 차단 |
| 수정 | 자기 자신 제외 동일 부서명 차단 |
| 삭제 | 소속 재직 직원 있으면 삭제 차단 |
...
| 연산 | 검증 조건 |
|------|-----------|
| 등록 | 직급명 중복 + Grade 중복 이중 체크 |
| 수정 | 자기 자신 제외 Grade 중복 체크 |
| 삭제 | 소속 재직 직원 있으면 삭제 차단 |
...
DB 레벨  : ATTEND_IN / ATTEND_OUT 프로시저 내부에서 재차 차단
...
### ⑥ 업무 등록·조회 및 업무 지시 발송·수신

#### 개요
회사 업무·부서 업무를 등록·조회하고, 상위 직급 직원이 하위 직급 직원에게 업무를 지시합니다.
...
| 메서드 | URL | 설명 |
|--------|-----|------|
| GET | `/worktargetlist.do` | 지시 대상자 목록 JSON |
| GET | `/work-submit/list` | 내가 발송한 업무 목록 |
| GET | `/work-receive/list` | 내가 수신한 업무 목록 |

#### 주요 구현 포인트

**업무 등록**

- 회사 업무: `WORK` 테이블 (코드 `WK0001~` 자동생성), 중복 등록 차단
- 부서 업무: `WORK_DEPT_RULE` 테이블 (FLOW 순서 포함) — 업무 지시 결재 라인 기반
- `VIEW_WORK_SEARCH` 뷰: 업무 + 등록자 부서·이름·직급을 한 번에 조회

**업무 지시**

- 대상자 조건: 같은 부서 + 자신보다 Grade가 낮은 재직자 (`VIEW_WORK_TARGET` 활용)

```
Grade=30 직원 → VIEW_WORK_TARGET → 같은 부서 & Grade < 30 재직자 목록 반환
```

- 발송 목록: `SH_DOCUMENT` 테이블 (`TYPE_CD='001'`), Java Stream으로 페이징
- 수신 목록: `WORK_TARGET` 테이블에서 내 `EMP_CD`가 수신자로 등록된 것 조회
